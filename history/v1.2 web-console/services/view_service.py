    ...
    return [
        {
            ...
            "button_text": build_action_button_text(action_name, action_info),
            "group": (
                "positive"
...
def build_action_button_text(action_name, action_info):
    energy_change = action_info["energy_change"]
    exp_change = action_info.get("exp_change", 0)
    text = f"{action_name} {energy_change:+}能量"
    if exp_change:
        text += f" / +{exp_change}经验"
