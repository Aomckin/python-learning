...
class GameCoreTest(unittest.TestCase):
    ...
    def test_complete_timed_action_applies_action_result_and_progression(self):
        core = GameCore(today_func=lambda: "2026-06-27")
        option = ActionDurationOption(minutes=60, multiplier=99, exp=9999)
    ...
    def test_complete_action_uses_configured_exp_change(self):
        data = json.loads(Path("actions.json").read_text(encoding="utf-8"))
        data["学习"]["exp_change"] = 4
        self.write_json("actions.json", data)
        core = GameCore(today_func=lambda: "2026-06-27")

        result = core.execute(GameCommand(COMPLETE_ACTION, {"action_name": "学习"}))
        saved = json.loads(Path("save.json").read_text(encoding="utf-8"))

        self.assertTrue(result.success)
        self.assertEqual(saved["energy"], 60)
        self.assertEqual(saved["exp"], 14)

    def test_action_fails_when_energy_is_not_enough(self):
        save_data = json.loads(Path("save.json").read_text(encoding="utf-8"))
        save_data["energy"] = 5
        self.write_json("save.json", save_data)
        core = GameCore(today_func=lambda: "2026-06-27")

        result = core.execute(GameCommand(COMPLETE_ACTION, {"action_name": "看番"}))
        saved = json.loads(Path("save.json").read_text(encoding="utf-8"))

        self.assertFalse(result.success)
        self.assertEqual(saved["energy"], 5)
        self.assertEqual(saved["exp"], 10)
        self.assertEqual(saved["action_counts"], {})
        self.assertFalse(Path("log.txt").exists())

    def test_timed_action_with_configured_positive_exp_is_data_driven(self):
        data = json.loads(Path("actions.json").read_text(encoding="utf-8"))
        data["学习"]["exp_change"] = 3
        self.write_json("actions.json", data)
        core = GameCore(today_func=lambda: "2026-06-27")
        option = ActionDurationOption(minutes=60, multiplier=2, exp=0)

        result = core.execute(
            GameCommand(
                COMPLETE_TIMED_ACTION,
                {"action_name": "学习", "option": option}
            )
        )
        saved = json.loads(Path("save.json").read_text(encoding="utf-8"))

        self.assertTrue(result.success)
        self.assertEqual(result.events[0].payload["action_result"]["exp_change"], 6)
        self.assertEqual(saved["exp"], 16)
