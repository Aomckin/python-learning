...
from core_event import ACTION_DURATION_OPTIONS, ERROR, GameEvent
        ...
        return OperationResult(
            ...
            events=[
                GameEvent(
                    ...
                    {"options": get_action_duration_options(self.actions[action_name])}
                )
            ],
            state=self.get_state()
        )

    def get_duration_option(self, action_name, option):
        if action_name not in self.actions or option is None:
            return None

        minutes = getattr(option, "minutes", None)
        for allowed in get_action_duration_options(self.actions[action_name]):
            if allowed.minutes == minutes:
                return allowed
