...
from clients.api_client import ApiClient, ApiClientError
from core import ActionTimerState
...
class GameUI:
    def __init__(self):
        ...
        try:
            initial_state = self.api_client.get_state()
        except ApiClientError as exc:
            root = tk.Tk()
            root.withdraw()
            messagebox.showerror("后端连接失败", str(exc))
            root.destroy()
        ...
        result = self.execute_command(INITIALIZE_PROGRESSION)
    ...
    def execute_command(self, command_type, payload=None):
        try:
            return self.api_client.execute_command(command_type, payload)
        except ApiClientError as exc:
            self.show_api_error(exc)
            return None

    def show_api_error(self, error):
        message = str(error)
        if hasattr(self, "level_notice_label"):
            self.level_notice_label.config(text=message)
        else:
            messagebox.showerror("后端连接失败", message)
    ...
    def open_action_duration_window(self, action_name):
        try:
            result = self.api_client.get_action_duration_options(action_name)
        except ApiClientError as exc:
            self.show_api_error(exc)
            return
        ...
        options = result.events[0].payload["options"] if result.events else []
    ...
    def handle_operation_result(self, result):
        ...
        self.update_display(result.state if result.get("state") else None)
    ...
    def settle_action_immediately(self, action_name):
        self.handle_operation_result(
            self.execute_command(COMPLETE_ACTION, {"action_name": action_name})
        )

    def handle_task(self, index):
        self.handle_operation_result(
            self.execute_command(COMPLETE_DAILY_TASK, {"index": index})
        )
    ...
    def handle_special_task(self, index):
        self.handle_operation_result(
            self.execute_command(COMPLETE_SPECIAL_TASK, {"index": index})
        )
    ...
    def handle_buy_shop_item(self, item_id):
        self.handle_operation_result(
            self.execute_command(BUY_SHOP_ITEM, {"item_id": item_id})
        )
        self.refresh_shop_window()
    ...
    def get_achievement_condition_text(self, achievement):
        for section in self.get_state().achievement_sections:
            for achievement_view in section["items"]:
    ...
    def handle_equip_title(self, title_id, window):
        result = self.execute_command(EQUIP_TITLE, {"title_id": title_id})
        if result.success:
            self.update_display()
            window.destroy()
    ...
    def get_title_condition_text(self, title):
        for title_view in self.get_state().title_views:
