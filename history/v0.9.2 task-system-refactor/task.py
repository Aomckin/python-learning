...
class SpecialTaskManager:
    ...
    def load_tasks(self):
        ...
        self.tasks_data.setdefault("active_task_id", "")
    ...
    def finish_task(self):
        ...
        self.task.done = True
        self.task.completed_count += 1

        self.save_tasks()

        return self.task.coin, self.task.exp
    ...
    def draw_daily_task(self):
        ...
        if len(task_ids) == 0:
            self.tasks_data["active_task_id"] = ""
            return

        self.tasks_data["active_task_id"] = random.choice(task_ids)

    def has_valid_active_task(self):
        active_task_id = self.tasks_data.get("active_task_id", "")
        task_ids = {task.id for task in self.all_tasks}

        return active_task_id in task_ids
