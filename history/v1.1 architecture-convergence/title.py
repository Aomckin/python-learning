...
from services.player_service import (
    equip_title as service_equip_title,
    unlock_title as service_unlock_title,
)
...
class TitleSystem(UnlockableSystem):
    def __init__(self, player, title_file="titles.json", special_tasks_file="special_tasks.json"):
        ...
        super().__init__(player, title_file)
        self.title_file = self.data_file
        self.titles = self.items
    ...
    def is_done(self, item, context=None):
        return self.is_title_done(item, context or [])
    ...
    def get_special_task_completed_count(self, task_id):
        if not file_exists(self.special_tasks_file):
            return 0

        tasks_data = load_json(self.special_tasks_file)
    ...
    def unlock_title_by_id(self, title_id):
        ...
        service_unlock_title(self.player, title_id)
        if not self.player.equipped_title:
            service_equip_title(self.player, title_id)
